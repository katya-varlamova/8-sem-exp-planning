...
double
uniform_real( double a, double b )
{
...
double
weibull_real( double k, double lambda )
{
    static thread_local std::mt19937 generator( std::random_device{}() );
...
uniform_mean_and_variance( double a, double b )
{
...
std::tuple<double, double>
weibull_mean_and_variance( double k, double lambda )
{
    const double mean = lambda * tgamma( 1.0 + 1.0 / k );
    const double variance =
      pow( lambda, 2 ) * tgamma( 1.0 + 2.0 / k ) - pow( mean, 2 );
    return { mean, variance };
}

std::tuple<double, double>
uniform_parameters_from_mean_and_std( double mean, double std )
{
...
std::tuple<double, double>
weibull_parameters_from_mean_and_std( double mean, double std )
{
...
std::tuple<double, double>
weibull_parameters_from_mean( double mean )
{
    // std ~=~ mean / 2;
    const double k = 2.0;
    const double lambda = mean / tgamma( 1 + 1 / k );
