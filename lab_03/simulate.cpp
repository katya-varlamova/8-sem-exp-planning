...
{
    return std::equal_to<double>{}( lhs.time, rhs.time ) &&
           lhs.type == rhs.type;
}
...
public:
    bool Receive( const Event& /* next_event */ ) override
    {
        throw std::runtime_error(
          "DummyConsumer->Receive(const Event&) should never be called"
        );
        return true;
    }
};
...
public:
    ...
    {
        throw std::runtime_error(
          "DummyProducer->Process(const Event&) should never be called"
        );
        return true;
    }
...
public:
    ...
    [[nodiscard]] Event GetNextEvent() const override { return next_event_; }

    [[nodiscard]] size_t GetNProcessed() const { return n_processed_; }

    [[nodiscard]] double GetAverageWaitingTime() const
    {
        return std::accumulate(
                 waiting_times_.begin(), waiting_times_.end(), 0.0
               ) /
...
private:
    ...
    {
        next_event_ = { current_event.time +
                          distributions_[current_event.type](),
                        current_event.type };
    }
...
{
public:
    [[maybe_unused]] std::shared_ptr<IDevice> AddDevice(
    ...
    {
        for ( auto current_event = FindCurrentEvent();
              current_event.time <= until;
              current_event = FindCurrentEvent() ) {
...
SimulateResult
Simulate( const SimulateParams& params )
{
    Simulator simulator;
    simulator.AddDevice( std::make_shared<RequestGenerator>(
      [=]() { return uniform_real( params.a1, params.b1 ); }, 1
    ) );
    simulator.AddDevice( std::make_shared<RequestGenerator>(
      [=]() { return uniform_real( params.a2, params.b2 ); }, 2
    ) );
    simulator.AddDevice(
      std::make_shared<RequestProcessor>( RequestProcessor::Distributions{
    ...
    auto processor =
      std::dynamic_pointer_cast<RequestProcessor>( simulator.GetDevice( 2 ) );
    return {
