...
ToDouble( const std::vector<std::vector<int>>& m )
{
    std::vector<std::vector<double>> res;
    for ( size_t i = 0; i < m.size(); i++ ) {
        res.push_back( std::vector<double>{} );
        for ( size_t j = 0; j < m[i].size(); j++ ) {
            res[i].push_back( double( m[i][j] ) );
        }
    }
...
std::vector<std::vector<double>>
Transpose( const std::vector<std::vector<double>>& m )
{
    std::vector<std::vector<double>> res;
    for ( size_t i = 0; i < m[0].size(); i++ ) {
        res.push_back( std::vector<double>{} );
        for ( size_t j = 0; j < m.size(); j++ ) {
            res[i].push_back( double( m[j][i] ) );
        }
    }
...
)
{
    ...
    for ( size_t i = 0; i < l.size(); i++ ) {
        ...
        for ( size_t j = 0; j < r[0].size(); j++ ) {
            ...
            for ( size_t z = 0; z < r.size(); z++ )
                accum += l[i][z] * r[z][j];

            res[i].push_back( accum );
        }
    }
...
CreateUnitPlanningMatrixCore( size_t k )
{
...
template<typename T>
T
CalculateProductOfCombinationInRow(
...
void
ProceedUnitPlanningMatrixCore( size_t k, std::vector<std::vector<int>>& ones )
{
    const auto N = static_cast<size_t>( std::pow( 2ul, k ) );
    const auto M = N - 1;
    ...
    for ( size_t n = 0; n < ones.size(); ++n ) {
        ...
        for ( size_t i = 2; i <= k; ++i ) {
            for ( auto&& combination : GenerateAllCombinations( k, i ) ) {
                ones[n].push_back(
                  CalculateProductOfCombinationInRow( combination, ones[n] )
                );
            }
        }
    }
...
std::vector<std::vector<int>>
CreateUnitPlanningMatrix( size_t k )
{
...
template<size_t k>
PartialNonlinearCoefficients<k>
CalculateCoefficients( const FfeTable& table )
{
    PartialNonlinearCoefficients<k> coefficients{};
    const size_t N = std::min( coefficients.N(), table.size() );

    const auto ones = N == coefficients.N() ? CreateUnitPlanningMatrix( k )
                                            : CreateUnitPlanningMatrix2();
    for ( size_t i = 0; i < N; ++i ) {
...
template<size_t k>
double
CalculatePartialNonlinearRegression(
  const PartialNonlinearCoefficients<k>& coefficients,
...
)
{
    ...
    for ( size_t i = 0, j = 0; i <= limit; ++i ) {
        for ( auto&& combination : GenerateAllCombinations( k, i ) ) {
            y += coefficients[j++] *
                 CalculateProductOfCombinationInRow( combination, factors );
        }
    }
...
template<size_t k>
using TCalculateRegression = std::function<double(
  const PartialNonlinearCoefficients<k>& coefficients,
  const std::vector<double>& factors
)>;
...
template<size_t k>
std::vector<double>
CalculateYUsingRegression(
  const PartialNonlinearCoefficients<k>& coefficients,
  TCalculateRegression<k>&& calculate_regression
)
{
...
template<size_t k>
std::vector<double>
CalculateYWithPartialNonlinearRegression(
  const PartialNonlinearCoefficients<k>& coefficients,
  size_t limit = k
)
{
    return CalculateYUsingRegression<6>(
      coefficients,
      [limit]( const auto& c, const auto& f ) {
          return CalculatePartialNonlinearRegression( c, f, limit );
      }
    );
}
...
template<size_t k>
std::vector<double>
CalculateYWithLinearRegression(
  const PartialNonlinearCoefficients<k>& coefficients
)
{
    return CalculateYUsingRegression<k>(
      coefficients,
      []( const auto& c, const auto& f ) {
          return CalculatePartialNonlinearRegression( c, f, 1 );
      }
    );
}

std::vector<double>
SimulateNTimes( const SimulateParams& params, size_t times )
{
    std::vector<double> y;
...
std::vector<double>
CalculateY( const DotParameters& dot_params, size_t times )
{
    const auto [a1, b1] = uniform_parameters_from_mean_and_std(
      1.0 / dot_params.lambda1, dot_params.sigma_lambda1
    );
    const auto [a2, b2] = uniform_parameters_from_mean_and_std(
      1.0 / dot_params.lambda2, dot_params.sigma_lambda2
    );
...
FfeResult
FullFactorialExperiment( FfeParameters params )
{
    FfeResult result;
    for ( size_t i = 0; i < 64u; ++i ) {
        ...
        result.table.push_back( {
          .index = i + 1,
          .x1 = params.lambda1.Norm( dot_params.lambda1 ),
          .x2 = params.lambda2.Norm( dot_params.lambda2 ),
          .x3 = params.mu1.Norm( dot_params.mu1 ),
          .x4 = params.mu2.Norm( dot_params.mu2 ),
          .x5 = params.sigma_lambda1.Norm( dot_params.sigma_lambda1 ),
          .x6 = params.sigma_lambda2.Norm( dot_params.sigma_lambda2 ),
    ...
    const auto u_hat =
      CalculateYWithPartialNonlinearRegression( result.coefficients, 6 );
...
FfeResult
FractionalFactorialExperiment( FfeParameters params )
{
    ...
    for ( size_t i = 0; i < xs.size(); ++i ) {
        const DotParameters dot_params = {
            ...
            .sigma_lambda2 =
              params.sigma_lambda2.Choose( xs[i][5] == 1 ), // x1*x2*x3*x4*x5
        };
    ...
    const auto y_hat = CalculateYWithLinearRegression( result.coefficients );
    const auto u_hat =
      CalculateYWithPartialNonlinearRegression( result.coefficients, 2 );
    for ( size_t i = 0; i < xs.size(); ++i ) {
        result.table[i].y_hat = y_hat[i];
        result.table[i].dy_hat = std::abs( result.table[i].y_mean - y_hat[i] );
        result.table[i].u_hat = u_hat[i];
...
std::vector<double>
NormalizeFactors( FfeParameters ffe_params, const DotParameters& dot_params )
{
    return {
...
double
CalculateDotWithRegression(
  const PartialNonlinearCoefficients<6>& coefficients,
  const std::vector<double>& factors,
  size_t limit
)
{
...
double
CalculateDot( const DotParameters& dot_params, size_t times )
{
