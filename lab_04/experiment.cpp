...
public:
    ...
    double& operator[]( size_t i ) { return a_[i]; }
    ...
    double CalculateRegression( const std::vector<double>& factors ) const
    {
        ...
        const auto S = std::sqrt(
          static_cast<double>( Nkernel ) / static_cast<double>( N )
        );
...
private:
    ...
    double aij( size_t i, size_t j ) const
    {
        ...
        const size_t index =
          1 + n + CalculateIndexOfCombination( n, indices_array ) - 1;
        return a_[index];
    }
...
public:
    OrthogonalCentralCompositeDesign()
      ...
      , S_{ std::sqrt(
          static_cast<double>( Nkernel_ ) / static_cast<double>( N_ )
        ) }
      , alpha_{ std::sqrt(
          static_cast<double>( Nkernel_ ) / 2.0 * ( 1.0 / S_ - 1.0 )
        ) }
    ...
    std::vector<double> CalculateYUsingRegression() const
    {
        ...
        std::transform(
          unit_planning_matrix_.begin(),
          unit_planning_matrix_.end(),
          std::back_inserter( y_hat ),
...
private:
    ...
    const std::vector<std::vector<size_t>>
      all_2_combinations_of_n_; // множество всех сочетаний из n по 2

    std::vector<std::vector<double>>
      unit_planning_matrix_; // безразмерная матрица планирования без столбцов
                             // "№", "x0" и "y"...
    ...
    void ProceedUnitPlanningMatrixCore()
    {
        for ( auto&& row : unit_planning_matrix_ ) {
            ...
            for ( auto&& combination : all_2_combinations_of_n_ ) {
                row.push_back(
                  CalculateProductOfCombinationInRow( combination, row )
                ); // xi * xj
            }
...
std::vector<std::vector<double>>
CreateUnitPlanningMatrix()
{
    OrthogonalCentralCompositeDesign<6> oc;
    return oc.GetMatrix();
}
...
public:
    ...
    CalculateY( const DotParameters& dot_params, size_t times )
    {
        ...
        const auto [k1, l1] =
          weibull_parameters_from_mean( 1.0 / dot_params.mu1 );
        const auto [k2, l2] =
          weibull_parameters_from_mean( 1.0 / dot_params.mu2 );
    ...
    void InitializePlanningMatrix( const ExperimentParameters& params )
    {
        ...
        for ( size_t i = 0; i < occd_.N_; ++i ) {
            const DotParameters dot_params = {
                .lambda1 =
                  params.lambda1.Choose( occd_.unit_planning_matrix_[i][0] ),
                .lambda2 =
                  params.lambda2.Choose( occd_.unit_planning_matrix_[i][1] ),
                .mu1 = params.mu1.Choose( occd_.unit_planning_matrix_[i][2] ),
                .mu2 = params.mu2.Choose( occd_.unit_planning_matrix_[i][3] ),
                .sigma_lambda1 =
                  params.sigma_lambda1.Choose( occd_.unit_planning_matrix_[i][4]
                  ),
                .sigma_lambda2 =
                  params.sigma_lambda2.Choose( occd_.unit_planning_matrix_[i][5]
                  ),
            };

            const auto y =
              CalculateMean( CalculateY( dot_params, params.times ) );
            ...
            std::copy(
              occd_.unit_planning_matrix_[i].begin(),
              occd_.unit_planning_matrix_[i].end(),
              std::back_inserter( planning_matrix_.back() )
            );
        ...
        for ( size_t i = 0; i < occd_.N_; ++i ) {
            ...
            planning_matrix_[i].push_back( std::abs(
              planning_matrix_[i][1 + occd_.coefficients_.M_] - y_hat[i]
            ) );
        }
    }
...
ExperimentResult
RunExperiment( const ExperimentParameters& params )
{
...
std::vector<double>
NormalizeFactors(
  const ExperimentParameters& exp_params,
  const DotParameters& dot_params
)
{
    return {
