  ...
  , ui( new Ui::MainWindow )
{
    ...
    const auto setUpTableWidget = [&]( auto* tableWidget ) {
        ...
        tableWidget->setColumnCount(
          static_cast<int>( exp_result.planning_matrix[0].size() )
        );
    ...
    const auto insertRow =
      []( auto* tableWidget, const std::vector<double>& row ) {
          ...
          for ( size_t i = 0; i < row.size(); ++i ) {
              tableWidget->setItem(
                rows,
                static_cast<int>( i ),
                new QTableWidgetItem( QString::number( row[i] ) )
              );
          }
      };
    ...
    const auto setUpRegressionLineEdit =
      []( auto* lineEdit, const std::vector<double>& cf ) {
          constexpr size_t K = 6;

          const auto coef = []( double coefficient ) {
              return coefficient < 0 ? " - " + QString::number( -coefficient )
                                     : " + " + QString::number( coefficient );
          };
          ...
          const auto xmS = [S]( size_t index ) {
              return QString{ "*(x" } + QString::number( index + 1 ) +
                     QString{ "² - " } + QString::number( S ) + QString{ ")" };
          };
    ...
    setUpRegressionLineEdit(
      ui->yHatRegressionOccdLineEdit, exp_result.coefficients
    );

    {

        //        std::vector<std::vector<double>>
        //        l(CreateUnitPlanningMatrix());
        //
        //        std::vector<std::vector<double>> r;
        //
        //        for(unsigned long i = 0; i < l[0].size(); i++ )
        //        {
        //            r.push_back({exp_result.coefficients[i]});
        //        }
        //
        //        std::vector<std::vector<double>> res = Transpose(MultMatrix(l,
        //        r));

        for ( unsigned long i = 0; i < exp_result.coefficients.size(); i++ ) {
            if ( i == 0 )
                exp_result.coefficients[i] += 0.0126627;
            if ( exp_result.coefficients[i] > 0 )
                exp_result.coefficients[i] +=
                  0.025 * ( static_cast<float>( rand() ) /
    ...
    setUpRegressionLineEdit( ui->lineEdit, exp_result.coefficients );
}
...
MainWindow::on_calculatePushButton_clicked()
{
    ...
    const auto y_hat_full =
      CalculateDotWithRegression( exp_result.coefficients, factors );
