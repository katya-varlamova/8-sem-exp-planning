...
class PfeFrame(tk.Frame): 
    ...
    def count_one(self, lam, mu):
        print(self.lambda_min, lam, self.lambda_max, lam < self.lambda_min, lam > self.lambda_max)
        print(self.mu_min, mu, self.mu_max,  mu < self.mu_min, mu > self.mu_max)
        if lam < self.lambda_min or lam > self.lambda_max or mu < self.mu_min or mu > self.mu_max:
            print("Точка не входит в промежуток варьирования!")
    ...
    def run(self, lambda_min, lambda_max, mu_min, mu_max, count):
        ...
        y_lin = self.count_lin(self.x_table, b, 3) # (y=b0*x0+b1*x1+b2*x2)
        y_nl = self.count_lin(self.x_table, b, len(b)) # (y=b0*x0+b1*x1+b2*x2+b12*x12)
