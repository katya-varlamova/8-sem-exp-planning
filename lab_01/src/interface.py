...
def getModelingData(params_generator, params_handler, time):
    ...
    gov = ModelingGoverner(time, [source_generator1], [handler_generator1], 0)
    s = gov.event()
...
def read():
    ...
    global labRes
    labRes.config(text = "Загрузка системы: {:.3f} (теория) {:.3f} (факт)".format(params_handler[0]/params_generator[0],
                                                                        data[2]/data[1]))
